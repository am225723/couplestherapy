...
router.post("/style", async (req, res) => {
  try {
    ...
    const { style_type, discipline_approach, values_text, stress_areas } =
      req.body;
...
router.get("/styles/:couple_id", async (req, res) => {
  try {
    ...
    if (couple_id !== userAuth.coupleId) {
      return res
        .status(403)
        .json({ error: "Cannot view different couple parenting styles" });
    }
...
router.post("/agreement", async (req, res) => {
  try {
    ...
    if (!scenario || !agreed_approach) {
      return res
        .status(400)
        .json({ error: "scenario and agreed_approach are required" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to create agreement" });
  }
});
...
router.get("/agreements/:couple_id", async (req, res) => {
  try {
    ...
    if (couple_id !== userAuth.coupleId) {
      return res
        .status(403)
        .json({ error: "Cannot view different couple agreements" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to fetch agreements" });
  }
});
...
router.get("/couple-time/:couple_id", async (req, res) => {
  try {
    ...
    if (couple_id !== userAuth.coupleId) {
      return res
        .status(403)
        .json({ error: "Cannot view different couple time blocks" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to fetch couple time" });
  }
});
...
router.post("/stress-checkin", async (req, res) => {
  try {
    ...
    if (!stress_level || stress_level < 1 || stress_level > 10) {
      return res
        .status(400)
        .json({ error: "Valid stress_level (1-10) is required" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to submit check-in" });
  }
});
...
router.get("/stress-checkins/:couple_id", async (req, res) => {
  try {
    ...
    if (couple_id !== userAuth.coupleId) {
      return res
        .status(403)
        .json({ error: "Cannot view different couple stress check-ins" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to fetch check-ins" });
  }
});
