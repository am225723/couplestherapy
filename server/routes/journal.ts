...
router.post("/entries", async (req, res) => {
  try {
    ...
    const { entry_content, entry_mode, visibility, mood, is_favorite } = req.body;
    ...
    if (!['private', 'shared_with_partner', 'shared_with_therapist'].includes(visibility)) {
...
router.get("/entries", async (req, res) => {
  try {
    ...
    if (!visibility) {
      query = query.or(`author_id.eq.${authResult.userId},visibility.in.(shared_with_partner,shared_with_therapist)`);
    } else if (visibility === 'private') {
      query = query.eq('author_id', authResult.userId).eq('visibility', 'private');
    } else {
...
router.get("/entries/:id", async (req, res) => {
  try {
    ...
    const canView = data.author_id === authResult.userId 
...
router.patch("/entries/:id", async (req, res) => {
  try {
    ...
    if (visibility === 'shared_with_therapist' && entry.visibility !== 'shared_with_therapist') {
      const { data: couple } = await supabaseAdmin
        .from('Couples_couples')
        .select('therapist_id')
...
router.post("/milestones", async (req, res) => {
  try {
    ...
    const { milestone_date, milestone_title, milestone_description, entry_id } = req.body;
