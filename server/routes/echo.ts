...
router.post("/session", async (req, res) => {
  try {
    ...
    if (validatedData.couple_id !== userAuth.coupleId) {
      return res
        .status(403)
        .json({ error: "Cannot create session for different couple" });
    }
...
router.post("/turn", async (req, res) => {
  try {
    ...
    if (session.couple_id !== userAuth.coupleId) {
      return res
        .status(403)
        .json({ error: "Cannot add turn to different couple session" });
    }
...
router.patch("/session/:id/complete", async (req, res) => {
  try {
    ...
    if (session.couple_id !== userAuth.coupleId) {
      return res
        .status(403)
        .json({ error: "Cannot complete different couple session" });
    }
