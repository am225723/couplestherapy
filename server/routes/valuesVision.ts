...
router.get("/dreams/:couple_id", async (req, res) => {
  try {
    ...
    if (couple_id !== userAuth.coupleId) {
      return res
        .status(403)
        .json({ error: "Cannot view different couple dreams" });
    }
...
router.get("/vision-board/:couple_id", async (req, res) => {
  try {
    ...
    if (couple_id !== userAuth.coupleId) {
      return res
        .status(403)
        .json({ error: "Cannot view different couple vision board" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to fetch vision board" });
  }
});
...
router.get("/core-values/:couple_id", async (req, res) => {
  try {
    ...
    if (couple_id !== userAuth.coupleId) {
      return res
        .status(403)
        .json({ error: "Cannot view different couple values" });
    }
