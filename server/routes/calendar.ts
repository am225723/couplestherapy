...
router.get("/:couple_id", async (req, res) => {
  try {
    ...
    if (!hasAccess) {
      return res
        .status(403)
        .json({ error: "Access denied to view this calendar." });
    }
...
router.patch("/:event_id", async (req, res) => {
  try {
    ...
    if (existingEvent.couple_id !== coupleId) {
      return res
        .status(403)
        .json({ error: "Access denied to update this event" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to update calendar event" });
  }
});
...
router.delete("/:event_id", async (req, res) => {
  try {
    ...
    if (existingEvent.couple_id !== coupleId) {
      return res
        .status(403)
        .json({ error: "Access denied to delete this event" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to delete calendar event" });
  }
});
