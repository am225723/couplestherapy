...
router.post("/incident", async (req, res) => {
  try {
    ...
    if (
      !horseman_type ||
      !["criticism", "contempt", "defensiveness", "stonewalling"].includes(
        horseman_type,
      )
    ) {
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to create incident" });
  }
});
...
router.get("/:couple_id", async (req, res) => {
  try {
    ...
    if (couple_id !== userAuth.coupleId) {
      return res
        .status(403)
        .json({ error: "Cannot view different couple incidents" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to fetch incidents" });
  }
});
...
router.patch("/:id/validate", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to validate incident" });
  }
});
...
router.patch("/:id/antidote", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to update antidote" });
  }
});
