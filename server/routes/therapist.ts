...
therapistRouter.get("/export-couple-report", async (req: Request, res: Response) => {
  try {
...
therapistRouter.post("/create-therapist", async (req: Request, res: Response) => {
  try {
    // Verify session and therapist authorization
    const authResult = await verifyTherapistSession(req);
    if (!authResult.success) {
...
therapistRouter.post("/regenerate-join-code", async (req: Request, res: Response) => {
  try {
    // Verify session and therapist authorization
    const authResult = await verifyTherapistSession(req);
    if (!authResult.success) {
...
therapistRouter.get("/unassigned-couples", async (req: Request, res: Response) => {
  try {
    // Verify session and therapist authorization
    const authResult = await verifyTherapistSession(req);
    if (!authResult.success) {
