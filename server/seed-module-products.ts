...
async function seedProducts() {
  ...
  for (const module of MODULES) {
    ...
    const existingMonthly = monthlyPrices.data.find(
      (p) =>
        p.recurring?.interval === "month" && p.unit_amount === MONTHLY_PRICE,
    );
    ...
    if (!existingMonthly) {
      ...
      console.log(
        `  Created monthly price: ${monthlyPrice.id} ($${MONTHLY_PRICE / 100}/mo)`,
      );
    } else {
    ...
    if (!existingYearly) {
      ...
      console.log(
        `  Created yearly price: ${yearlyPrice.id} ($${YEARLY_PRICE / 100}/yr)`,
      );
    } else {
