...
function validateRequest(body: any): 
  ...
  | { valid: true; data: { session_id: string; turn_id: string; speaker_message: string; listener_response: string } }
{
  ...
  if (!speaker_message || typeof speaker_message !== 'string' || speaker_message.trim().length === 0) {
    return { valid: false, error: 'speaker_message is required and must be a non-empty string' };
  }

  if (speaker_message.length > 2000) {
    return { valid: false, error: 'speaker_message too long (max 2000 characters)' };
  }

  if (!listener_response || typeof listener_response !== 'string' || listener_response.trim().length === 0) {
    return { valid: false, error: 'listener_response is required and must be a non-empty string' };
  }

  if (listener_response.length > 2000) {
    return { valid: false, error: 'listener_response too long (max 2000 characters)' };
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { session_id, turn_id, speaker_message, listener_response } = validation.data;
    ...
    if (session.couple_id !== coupleId) {
      return new Response(
        JSON.stringify({ error: 'Unauthorized: Session does not belong to your couple' }),
    ...
    if (session.listener_id !== userId) {
      return new Response(
        JSON.stringify({ error: 'Unauthorized: You must be the listener in this session to receive coaching' }),
        { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (turnError || !turn || turn.session_id !== session_id) {
      return new Response(
        JSON.stringify({ error: 'Turn not found or does not belong to this session' }),
        { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
...
Be encouraging and constructive. Focus on growth.`;
    ...
    return new Response(
      JSON.stringify({ error: error.message || 'Failed to generate coaching feedback' }),
