...
import {
  getTherapistSecondaryLinks,
  getTherapistTabHref,
  routeToTherapistPrimaryTab,
  therapistTabConfig,
} from "@/config/therapistTabConfig";
...
interface TherapistTabShellProps {
...
export function TherapistTabShell({
  profile,
  onSignOut,
  children,
}: TherapistTabShellProps) {
  const [location] = useLocation();
  const activeTabId = routeToTherapistPrimaryTab(location);
  const secondaryLinks = getTherapistSecondaryLinks(activeTabId, location);
      ...
      <header className="sticky top-0 z-40 border-b border-border/70 bg-background/72 backdrop-blur-xl">
        <div className="mx-auto flex w-full max-w-[1800px] items-center gap-3 px-3 py-3 sm:px-5 md:px-6">
          ...
          <nav className="hidden min-w-0 flex-1 items-center justify-center gap-1 md:flex">
            {therapistTabConfig.map((tab) => {
              const Icon = tab.icon;
              const isActive = activeTabId === tab.id;
              const tabHref = getTherapistTabHref(tab.id, location);

              return (
                <Link key={tab.id} href={tabHref}>
                  <button
                    type="button"
                    className={cn(
                    ...
                    data-testid={`therapist-primary-tab-${tab.id}`}
                  >
          ...
          <div className="ml-auto flex items-center gap-2">
            <Button
              ...
              data-testid="button-therapist-notifications"
            >
                  ...
                  <Avatar className="h-9 w-9">
                    {profile.avatar_url ? (
                      <AvatarImage
                        ...
                        alt={profile.full_name || "Therapist"}
                      />
                    ) : null}
        ...
        <div className="hidden border-t border-border/50 px-3 py-2 sm:px-5 md:block md:px-6">
          <div className="mx-auto flex w-full max-w-[1800px] flex-wrap items-center gap-2">
            {secondaryLinks.map((link) => {
              ...
              const isActive = location === link.url || location.endsWith(link.url);
              return (
                <Link key={link.url} href={link.url}>
                  <button
                    ...
                    data-testid={`therapist-secondary-link-${link.url.replace(/\//g, "-")}`}
                  >
            ...
            return (
              <Link key={tab.id} href={tabHref}>
                <button
                  ...
                  data-testid={`therapist-mobile-tab-${tab.id}`}
                >
                  <Icon className="h-4 w-4" />
                  <span>{tab.label}</span>
                </button>
