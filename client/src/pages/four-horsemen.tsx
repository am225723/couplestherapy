...
export default function FourHorsemenPage() {
  ...
  const createIncidentMutation = useMutation({
    mutationFn: async (data: { horseman_type: HorsemanType; situation: string; notes: string }) => {
      const response = await apiRequest('POST', '/api/horsemen/incident', data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/horsemen', profile?.couple_id] });
      toast({
  ...
  const validateMutation = useMutation({
    mutationFn: async ({ id, partner_validated }: { id: string; partner_validated: boolean }) => {
      const response = await apiRequest('PATCH', `/api/horsemen/${id}/validate`, { partner_validated });
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/horsemen', profile?.couple_id] });
  ...
  const antidoteMutation = useMutation({
    mutationFn: async ({ id, antidote_practiced }: { id: string; antidote_practiced: boolean }) => {
      const response = await apiRequest('PATCH', `/api/horsemen/${id}/antidote`, { antidote_practiced });
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/horsemen', profile?.couple_id] });
  ...
  const antidotePracticed = incidents.filter(i => i.antidote_practiced).length;
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          The Four Horsemen are communication patterns that predict relationship distress. 
        ...
        <Card data-testid="card-antidote-success">
          ...
          <CardContent>
            ...
            <p className="text-sm text-muted-foreground">
              {antidotePracticed} of {totalIncidents} incidents used the antidote
            </p>
          </CardContent>
        </Card>
      )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select which horseman occurred, describe the situation, and commit to practicing the antidote.
          </CardDescription>
        </CardHeader>
          ...
          {selectedHorseman && (
            <>
              ...
              <Button
                ...
                className="w-full"
              >
                {createIncidentMutation.isPending ? 'Logging...' : 'Log Incident'}
              </Button>
            </>
          )}
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Track patterns and practice antidotes</CardDescription>
        </CardHeader>
            ...
            incidents.map((incident) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 pt-2">
                  {incident.partner_validated === null && incident.reporter_id !== profile?.id && (
                    <>
                      <Button
                        data-testid={`button-validate-yes-${incident.id}`}
                        size="sm"
                        variant="outline"
                        onClick={() => validateMutation.mutate({ id: incident.id, partner_validated: true })}
                      >
                      ...
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        onClick={() => validateMutation.mutate({ id: incident.id, partner_validated: false })}
                      >
                  ...
                  {incident.partner_validated !== null && (
                    <Badge variant={incident.partner_validated ? "default" : "secondary"}>
                      Partner {incident.partner_validated ? 'validated' : 'disagreed'}
                    </Badge>
                  )}
                  ...
                  {!incident.antidote_practiced && (
                    <Button
                      ...
                      size="sm"
                      onClick={() => antidoteMutation.mutate({ id: incident.id, antidote_practiced: true })}
                    >
                      <CheckCircle className="w-4 h-4 mr-1" />
                      I practiced the antidote
                    </Button>
                  )}
