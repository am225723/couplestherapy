...
export default function IfsIntroPage() {
  ...
  const activeExercise = exercises?.find(e => e.status === 'in_progress') || null;
  ...
  const addPartMutation = useMutation({
    mutationFn: async (partData: { part_name: string; when_appears: string; letter_content: string }) => {
    ...
    onSuccess: () => {
      ...
      toast({
        title: 'Part Added',
        description: 'Your protective part has been identified and acknowledged.',
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const handleUpdatePart = () => {
    if (!editingPart || !partName.trim() || !whenAppears.trim() || !letterContent.trim()) {
      toast({
        title: 'Missing Information',
        description: 'Please fill in all fields.',
        variant: 'destructive',
  ...
  return (
    <div className="container max-w-5xl mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold mb-2" data-testid="text-page-title">IFS Introduction</h1>
        <p className="text-muted-foreground" data-testid="text-page-description">
          Inner Family Systems (IFS) helps you identify and understand your protective parts. These are inner voices that try to keep you safe, but may sometimes hold you back.
        </p>
      </div>
      ...
      <Card data-testid="card-info">
        ...
        <CardContent className="space-y-2 text-sm">
          <p data-testid="text-info-1">
            <strong>Examples of protective parts:</strong> The Critic, The Perfectionist, The People Pleaser, The Controller, The Avoider
          </p>
          <p data-testid="text-info-2">
            <strong>How to identify them:</strong> Notice inner voices that judge, control, or protect you in specific situations.
          </p>
          <p data-testid="text-info-3">
            <strong>This is private:</strong> Your letters to your parts are visible only to you and your therapist, not your partner.
          </p>
        </CardContent>
      </Card>
      ...
      <div className="flex justify-center">
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          ...
          <DialogContent className="max-w-2xl" data-testid="dialog-add-part">
            <DialogHeader>
              <DialogTitle data-testid="text-dialog-title">Identify a Protective Part</DialogTitle>
              <DialogDescription data-testid="text-dialog-description">
                Follow the three steps to identify and acknowledge a protective part.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-6 py-4">
              {/* Step 1: Name the Part */}
              <div className="space-y-2">
                <Label htmlFor="part-name" className="text-base font-semibold" data-testid="label-part-name">
                  Step 1: Name This Part
                </Label>
                <p className="text-sm text-muted-foreground" data-testid="text-part-name-hint">
              ...
              <div className="space-y-2">
                <Label htmlFor="when-appears" className="text-base font-semibold" data-testid="label-when-appears">
                  Step 2: When Does This Part Show Up?
                </Label>
                <p className="text-sm text-muted-foreground" data-testid="text-when-appears-hint">
              ...
              <div className="space-y-2">
                <Label htmlFor="letter-content" className="text-base font-semibold" data-testid="label-letter">
                  Step 3: Write a Letter to This Part
                </Label>
                <p className="text-sm text-muted-foreground" data-testid="text-letter-hint">
                  Thank this part for trying to protect you. Acknowledge what it's trying to do, even if it's not always helpful.
                </p>
                <Textarea
                  id="letter-content"
                  value={letterContent}
      ...
      {/* Edit Part Dialog */}
      {editingPart && (
        <Dialog open={!!editingPart} onOpenChange={(open) => !open && handleCancelEdit()}>
          <DialogContent className="max-w-2xl" data-testid="dialog-edit-part">
            <DialogHeader>
              <DialogTitle data-testid="text-edit-dialog-title">Edit Protective Part</DialogTitle>
            ...
            <div className="space-y-6 py-4">
              <div className="space-y-2">
                <Label htmlFor="edit-part-name" className="text-base font-semibold" data-testid="label-edit-part-name">
                  Part Name
                </Label>
                <Input
                  id="edit-part-name"
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-when-appears" className="text-base font-semibold" data-testid="label-edit-when-appears">
                  When It Appears
                </Label>
                <Textarea
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-letter-content" className="text-base font-semibold" data-testid="label-edit-letter">
                  Letter to This Part
                </Label>
                <Textarea
            ...
            <DialogFooter>
              ...
              >
                {updatePartMutation.isPending && <Loader2 className="h-4 w-4 animate-spin mr-2" />}
                Update Part
              </Button>
            </DialogFooter>
          </DialogContent>
      ...
      <div>
        <h2 className="text-2xl font-bold mb-4" data-testid="text-parts-title">Your Protective Parts</h2>
        {activeExercise && activeExercise.parts && activeExercise.parts.length > 0 ? (
              ...
              <Card key={part.id} data-testid={`card-part-${part.id}`}>
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <CardTitle className="flex items-center gap-2" data-testid={`text-part-name-${part.id}`}>
                        <Heart className="h-5 w-5" />
                        {part.part_name}
                      </CardTitle>
                      <CardDescription data-testid={`text-part-date-${part.id}`}>
                    ...
                    <div className="flex gap-2">
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent data-testid={`dialog-delete-confirm-${part.id}`}>
                          <AlertDialogHeader>
                            <AlertDialogTitle>Delete this part?</AlertDialogTitle>
                            <AlertDialogDescription>
                              This will permanently delete "{part.part_name}" and your letter to it. This action cannot be undone.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel data-testid={`button-cancel-delete-${part.id}`}>Cancel</AlertDialogCancel>
                            <AlertDialogAction
                ...
                <CardContent className="space-y-4">
                  <div data-testid={`container-when-appears-${part.id}`}>
                    ...
                    <p className="text-sm text-muted-foreground" data-testid={`text-when-appears-${part.id}`}>
                      {part.when_appears}
                    </p>
                  </div>
                  <div data-testid={`container-letter-${part.id}`}>
                    <p className="text-sm font-semibold mb-1">Your Letter to This Part:</p>
                    <div className="bg-muted p-4 rounded-md">
                      <p className="text-sm whitespace-pre-wrap" data-testid={`text-letter-${part.id}`}>
                        {part.letter_content}
                      </p>
                    </div>
                  </div>
          ...
          <Card data-testid="card-no-parts">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-muted-foreground mb-4" data-testid="text-no-parts">
                You haven't identified any protective parts yet.
              </p>
              <Button onClick={handleStartExercise} data-testid="button-start-first-part">
