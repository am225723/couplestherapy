...
export default function CoupleCompatibility() {
  ...
  const fetchAssessments = async () => {
    ...
    try {
      ...
      if (enneagrams) {
        ...
        if (p1Enn)
          data.partner1.enneagramType = `${p1Enn.dominant_type}/${p1Enn.secondary_type}`;
        if (p2Enn)
          data.partner2.enneagramType = `${p2Enn.dominant_type}/${p2Enn.secondary_type}`;
      }
  ...
  const generateInsights = (data: AssessmentData) => {
    ...
    if (data.partner1.attachmentStyle === data.partner2.attachmentStyle) {
      newInsights.push({
        ...
        description:
          "You both have similar attachment patterns, which can create understanding but also requires awareness of shared triggers.",
        icon: "🔗",
        type: "mindful",
      });
    } else {
      newInsights.push({
        ...
        description:
          "Your different attachment styles create balance. Understanding and appreciating these differences strengthens your bond.",
        icon: "⚖️",
        type: "strength",
      });
    }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold mb-2">Couples Compatibility</h1>
        <p className="text-muted-foreground">
          Understanding how you and your partner work together based on
          assessments
        </p>
      </div>
              ...
              <Card key={idx}>
                ...
                <CardContent>
                  ...
                  <Badge
                    className="mt-3"
                    variant={
                      insight.type === "strength" ? "default" : "secondary"
                    }
                  >
...
function SuggestionsCard({
  ...
  if (partner.loveLanguage?.primary === "Quality Time") {
    suggestions.push(
      `Schedule regular one-on-one time with ${otherPartner.name}. Even 15-30 minutes of undivided attention weekly strengthens your connection.`,
    );
  }
  if (partner.loveLanguage?.primary === "Words of Affirmation") {
    suggestions.push(
      `Share specific compliments and appreciation with ${otherPartner.name} regularly. Write notes or send messages highlighting what you admire.`,
    );
  }
  if (partner.loveLanguage?.primary === "Acts of Service") {
    suggestions.push(
      `Look for ways to help ${otherPartner.name} with tasks or responsibilities. Small gestures like making coffee or handling a chore speaks volumes.`,
    );
  }
  if (partner.loveLanguage?.primary === "Receiving Gifts") {
    suggestions.push(
      `Thoughtful gifts don't need to be expensive. ${otherPartner.name} will appreciate items that show you're thinking of them.`,
    );
  }
  if (partner.loveLanguage?.primary === "Physical Touch") {
    suggestions.push(
      `Increase physical affection with ${otherPartner.name}. Hugs, hand-holding, and cuddling strengthen emotional bonds.`,
    );
  }

  if (partner.attachmentStyle === "Secure") {
    suggestions.push(
      "Your secure attachment style is a strength—help ${otherPartner.name} feel safe and supported in the relationship.",
    );
  }
  if (partner.attachmentStyle === "Anxious") {
    suggestions.push(
      `Express your needs clearly to ${otherPartner.name}. Reassurance and consistent communication help you feel secure.`,
    );
  }
  if (partner.attachmentStyle === "Avoidant") {
    suggestions.push(
      `Work on vulnerability with ${otherPartner.name}. Sharing feelings and maintaining emotional connection strengthens your bond.`,
    );
  }
