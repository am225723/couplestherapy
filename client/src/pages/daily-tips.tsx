...
import { Loader2, AlertCircle, Lightbulb, RefreshCw, Settings } from 'lucide-react';
...
const CATEGORY_COLORS: Record<string, string> = {
  communication: 'bg-blue-100 text-blue-900 dark:bg-blue-900 dark:text-blue-100',
  intimacy: 'bg-pink-100 text-pink-900 dark:bg-pink-900 dark:text-pink-100',
  conflict: 'bg-orange-100 text-orange-900 dark:bg-orange-900 dark:text-orange-100',
  gratitude: 'bg-green-100 text-green-900 dark:bg-green-900 dark:text-green-100',
  connection: 'bg-purple-100 text-purple-900 dark:bg-purple-900 dark:text-purple-100',
  growth: 'bg-indigo-100 text-indigo-900 dark:bg-indigo-900 dark:text-indigo-100',
};
...
export default function DailyTips() {
  ...
  const generateMutation = useMutation({
    mutationFn: async (category: string) => {
      ...
      return apiRequest('POST', `/api/daily-tips/couple/${profile.couple_id}`, { category });
    },
    onSuccess: () => {
      toast({ title: 'Success', description: 'New tip generated!' });
      queryClient.invalidateQueries({ queryKey: [`/api/daily-tips/couple/${profile?.couple_id}/today`] });
      queryClient.invalidateQueries({ queryKey: [`/api/daily-tips/couple/${profile?.couple_id}`] });
    },
    onError: (error: any) => {
      toast({
        title: 'Error',
  ...
  const prefsMutation = useMutation({
    mutationFn: async (updates: Partial<NotificationPreferences>) => {
      ...
      return apiRequest('PATCH', `/api/daily-tips/preferences/${profile.couple_id}`, payload);
    },
    onSuccess: () => {
      toast({ title: 'Success', description: 'Preferences updated' });
      queryClient.invalidateQueries({ queryKey: [`/api/daily-tips/preferences/${profile?.couple_id}`] });
    },
  });
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">AI-powered insights to strengthen your connection</p>
      </div>
      ...
      {tipsQuery.isLoading ? (
        ...
        <Card className="bg-gradient-to-br from-primary/5 to-primary/10 border-primary/20">
          <CardHeader>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                <Badge className={CATEGORY_COLORS[todayTip.category] || CATEGORY_COLORS.connection}>
                  {todayTip.category.charAt(0).toUpperCase() + todayTip.category.slice(1)}
                </Badge>
              </div>
              <Button
                variant="outline"
          ...
          <CardContent>
            <p className="text-lg leading-relaxed text-foreground">{todayTip.tip_text}</p>
            <p className="text-xs text-muted-foreground mt-4">
              Generated on {new Date(todayTip.created_at).toLocaleDateString('en-US', {
                month: 'long',
                day: 'numeric',
                year: 'numeric',
              })}
        ...
        <Alert>
          ...
          <AlertDescription>No tip available yet. Click the button to generate one!</AlertDescription>
        </Alert>
      )}

      {/* Controls */}
      <div className="flex gap-3 flex-wrap">
        {['communication', 'intimacy', 'conflict', 'gratitude', 'connection', 'growth'].map(category => (
          <Button
            key={category}
            variant="outline"
          ...
          >
            {generateMutation.isPending && <Loader2 className="w-3 h-3 animate-spin mr-2" />}
      ...
      <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
        ...
        <DialogContent className="w-full max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>Customize how you receive daily tips</DialogDescription>
          </DialogHeader>
              ...
              {preferences.tips_enabled && (
                <>
                  <div>
                    ...
                    <Select value={preferences.tips_frequency} onValueChange={(value) => {
                      setPreferences({ ...preferences, tips_frequency: value });
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setPreferences({ ...preferences, tips_time: e.target.value });
                        prefsMutation.mutate({ tips_time: `${e.target.value}:00` });
                      }}
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => {
                    ...
                    prefsMutation.mutate({ push_notifications_enabled: checked });
                  }}
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => {
                    ...
                    prefsMutation.mutate({ email_notifications_enabled: checked });
                  }}
