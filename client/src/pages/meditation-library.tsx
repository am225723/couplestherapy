...
export default function MeditationLibraryPage() {
  ...
  const startSessionMutation = useMutation({
    mutationFn: async (meditation_id: string) => {
      const response = await apiRequest('POST', '/api/meditation/session', { meditation_id });
  ...
  const completeSessionMutation = useMutation({
    mutationFn: async ({ id, feedback }: { id: string; feedback: string }) => {
      const response = await apiRequest('PATCH', `/api/meditation/session/${id}/complete`, { feedback });
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/meditation/sessions', profile?.couple_id] });
      toast({
  ...
  const meditationsByCategory = meditations.reduce((acc, med) => {
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Guided meditations to deepen your connection, practice mindfulness, and nurture compassion.
        </p>
      </div>
        ...
        <Card className="border-2 border-primary">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="p-4 bg-muted rounded-md">
              <p className="text-sm">
                Find a comfortable position together or separately. Set aside this time for mindfulness and connection.
                When you're ready, click complete to reflect on your experience.
              </p>
            </div>
