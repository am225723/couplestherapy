...
export default function TherapistProfile() {
  ...
  useEffect(() => {
    // Try to load avatar URL from profile if it exists
  ...
  const handleSaveCrop = async () => {
    ...
    try {
      ...
      if (avatarUrl) {
        ...
        if (oldPath) {
          try {
            await supabase.storage.from("therapist-avatars").remove([oldPath]);
          } catch (error) {
      ...
      if (uploadError) {
        if (uploadError.message.includes("Bucket not found")) {
          throw new Error(
            "Storage bucket not found. Please create the 'therapist-avatars' bucket in your Supabase Storage.",
          );
        }
        throw uploadError;
      }
