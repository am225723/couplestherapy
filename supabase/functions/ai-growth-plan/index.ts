...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const loveLanguages = loveLanguagesRes.ok
      ? await loveLanguagesRes.json()
      : [];
    ...
    if (attachments?.length) {
      personalizationContext.attachment_styles = attachments.map(
        (a: any) => a.attachment_style,
      );
    }
