...
function validateRequest(data: any): { valid: boolean; error?: string } {
  ...
  if (data.speaker_message.length > 2000) {
    return {
      valid: false,
      error: "speaker_message exceeds maximum length of 2000 characters",
    };
  }
  ...
  if (data.listener_response.length > 2000) {
    return {
      valid: false,
      error: "listener_response exceeds maximum length of 2000 characters",
    };
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { session_id, turn_id, speaker_message, listener_response } =
      rawBody as EchoCoachingRequest;
