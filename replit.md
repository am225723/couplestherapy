...
-   **Widget Content Customization:** Therapists can personalize cards for each couple via the "Card Content" tab in the dashboard customizer:
    - **"From Your Therapist" Card:** Custom title, description, and section visibility toggles (Messages, To-Dos, Resources)
    - **Quick Action Cards:** Custom title, description, and button text for "Date Night Generator" and "Check-In History" cards
    - Stored as `widget_content_overrides` jsonb in `Couples_dashboard_customization` table with sensible fallbacks
