...
-   **Session Notes Integration:** SQL migration ready (`supabase/migrations/session_notes_integration.sql`) that adds `Couples_session_notes` table for therapists to document sessions with fields for summary, key themes, interventions, homework, and progress notes. Links to-do items via `session_note_id` foreign key for therapist workflow integration.
