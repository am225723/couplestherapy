...
export default function PauseButtonScreen() {
  ...
  const triggerPause = useApiMutation(
    '/api/pause',
    'post',
    {
  ...
  const resolvePause = useApiMutation(
    '/api/pause/resolve',
    'post',
    {
      invalidateQueries: [`/api/pause/couple/${profile?.couple_id}/current`],
      onSuccess: () => {
        Alert.alert('Ready to Continue', 'Great job taking a break. Ready to reconnect?');
      },
    }
  );
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        ...
        <Card style={[styles.pauseCard, isCurrentlyPaused && styles.pausedCard]}>
          {isCurrentlyPaused ? (
            <>
              ...
              {currentPause.reason && (
                <Text style={styles.reasonText}>Reason: {currentPause.reason}</Text>
              )}
            ...
            <>
              ...
              <Text style={styles.instructionText}>
                When emotions run high, it's okay to take a break. This will notify your partner
                that you need some space to calm down before continuing the conversation.
              </Text>

              <Card style={styles.guidelinesCard}>
                <Text style={styles.guidelinesTitle}>When to use the Pause Button:</Text>
                <Text style={styles.guidelineText}>• When you feel overwhelmed</Text>
                <Text style={styles.guidelineText}>• When you're too angry to listen</Text>
                <Text style={styles.guidelineText}>• When the conversation is escalating</Text>
                <Text style={styles.guidelineText}>• When you need to collect your thoughts</Text>
              </Card>
            </>
          )}
        </Card>
...
const styles = StyleSheet.create({
  ...
  pausedTitle: { ...typography.h4, color: colors.text, marginBottom: spacing.sm },
  pausedText: { ...typography.body, color: colors.textSecondary, textAlign: 'center', marginBottom: spacing.md },
  reasonText: { ...typography.body, color: colors.text, textAlign: 'center', marginBottom: spacing.lg, fontStyle: 'italic' },
  cooldownTips: { marginBottom: spacing.lg, width: '100%' },
  tipsTitle: { ...typography.h6, color: colors.text, marginBottom: spacing.sm },
  tipText: { ...typography.body, color: colors.textSecondary, marginBottom: spacing.xs },
  instructionTitle: { ...typography.h4, color: colors.text, marginBottom: spacing.sm },
  instructionText: {
    ...typography.body,
  ...
  guidelinesCard: { marginTop: spacing.lg, width: '100%', backgroundColor: colors.background },
  guidelinesTitle: { ...typography.h6, color: colors.text, marginBottom: spacing.sm },
  guidelineText: { ...typography.body, color: colors.textSecondary, marginBottom: spacing.xs },
});
