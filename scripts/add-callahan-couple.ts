...
async function addCallahanCouple() {
  try {
    ...
    const { data: matthewAuth, error: matthewError } =
    ...
    const { data: karliAuth, error: karliError } =
      await supabaseAdmin.auth.admin.createUser({
    ...
    if (matthewProfileError) {
      ...
      throw new Error(
        `Failed to create Matthew's profile: ${matthewProfileError.message}`,
      );
    }
    ...
    if (karliProfileError) {
      ...
      await supabaseAdmin.auth.admin.deleteUser(karliAuth.user.id);
      throw new Error(
        `Failed to create Karli's profile: ${karliProfileError.message}`,
      );
    }
