...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (memoError || !memo) {
      return new Response(JSON.stringify({ error: "Voice memo not found" }), {
        status: 404,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
...
Be very gentle. Focus on the positive. Only suggest improvements if truly needed.`;
    ...
    for (const line of lines) {
      ...
      if (trimmedLine) {
        ...
        if (
          currentSection === "tone" &&
          !tone.match(/neutral|loving|appreciative|concerned|frustrated/i)
        ) {
          ...
          if (bulletMatch || subNumberMatch) {
            ...
            !trimmedLine.toLowerCase().includes("none")
          ) {
